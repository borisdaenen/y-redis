...
const logWorker: any = logging.createModuleLogger("@y/redis/api/worker");
const logApi: any = logging.createModuleLogger("@y/redis/api");
 ...
 * @param {string} prefix
 */
const extractMessagesFromStreamReply = (streamReply: any, prefix: string) => {
  /**
  ...
  streamReply?.forEach((docStreamReply) => {
    ...
    const docMessages: any = map.setIfUndefined(
    ...
    docStreamReply.messages.forEach((m) => {
      if (m.message.m != null) {
        (docMessages as any).messages.push(
          /** @type {Uint8Array} */ m.message.m as any
        );
      }
    });
  });
 ...
 */
export const createApiClient = async (store: any, redisPrefix: string) => {
  const a = new Api(store, redisPrefix);
  await a.redis.connect();
  try {
    await a.redis.xGroupCreate(
...
export class Api {
  ...
  redisMinMessageLifetime: number;
  redisWorkerStreamName: string;
  redisWorkerGroupName: string;
  _destroyed: boolean;
  redis: any;
  /**
   * @param {import('./storage.js').AbstractStorage} store
   ...
   */
  async getMessages(streams: any): Promise<any[]> {
    if (streams.length === 0) {
    ...
    reads?.forEach((stream) => {
      res.push({
        ...
        lastId: (array.last(stream.messages) as any).id.toString(),
      });
    });
    return res;
  }
   ...
   * @param {Buffer} m
   */
  addMessage(room: string, docid: string, m: any) {
   ...
   * @param {string} docid
   */
  async getStateVector(room: string, docid: string = "/") {
   ...
   * @param {string} docid
   */
  async getDoc(room: string, docid: string) {
    logApi(`getDoc(${room}, ${docid})`);
    const ms = extractMessagesFromStreamReply(
      await this.redis.xRead(redis.commandOptions({ returnBuffers: true }), {
        key: computeRedisRoomStreamName(room, docid, this.prefix),
        id: "0",
      }),
      this.prefix
    );
    ...
    await promise.all(
      tasks.map(async (task) => {
        ...
        if (streamlen === 0) {
          ...
          const { ydoc, storeReferences, redisLastId, docChanged } =
          ...
          await promise.all([
            ...
            this.redis
              ...
              .xReadGroup(
                this.redisWorkerGroupName,
                "pending",
                { key: this.redisWorkerStreamName, id: ">" },
                { COUNT: 50 }
