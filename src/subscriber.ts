 ...
 */
const run = async (subscriber: Subscriber) => {
  while (true) {
    try {
      const ms = await subscriber.client.getMessages(
        [...subscriber.subs.entries()].map(([stream, s]) => ({
          key: stream,
          id: s.id,
        }))
      );
...
type SubFn = (stream: string, messages: Uint8Array[]) => void;
type SubRecord = { fs: Set<SubFn>; id: string; nextId: string | null };
...
export class Subscriber {
  ...
  constructor(client) {
    this.client = client;
    this.subs = new Map<string, SubRecord>();
    run(this);
  }
